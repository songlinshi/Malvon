...
public var MAsuggestionCellNib = "MAFramworkDefaultCell"

public class MASuggestionsWindowController: NSWindowController {
    ...
    private var selectedView: NSView? {
        didSet {
            if selectedView != oldValue {
                (oldValue as? MAHighlightingView)?.setHighlighted(false)
            }
            (selectedView as? MAHighlightingView)?.setHighlighted(true)
        }
    }
    ...
    public func begin(for parentTextField: NSSearchField?) {
        ...
        (suggestionWindow as? MASuggestionsWindow)?.parentElement = unignoredAccessibilityDescendant
        (unignoredAccessibilityDescendant as? MASuggestibleTextFieldCell)?.suggestionsWindow = suggestionWindow
    ...
    public func cancelSuggestions() {
        ...
        if suggestionWindow?.isVisible ?? false {
            ...
            ((suggestionWindow as? MASuggestionsWindow)?.parentElement as? MASuggestibleTextFieldCell)?.suggestionsWindow = nil
            (suggestionWindow as? MASuggestionsWindow)?.parentElement = nil
        }
    ...
    private func layoutSuggestions() {
        ...
        let contentView = window?.contentView as? MARoundedCornersView
        ...
        for entry: [String: Any] in suggestions {
            ...
            var frameworkBundle: Bundle? {
                let bundleId = "com.ashwin.MASearchSuggestions"
                return Bundle(identifier: bundleId)
            }
            var viewController: NSViewController
            
            if MAsuggestionCellNib == "MAFramworkDefaultCell" {
                viewController = NSViewController(nibName: "MAFramworkDefaultCell", bundle: frameworkBundle)
            } else {
                viewController = NSViewController(nibName: MAsuggestionCellNib, bundle: nil)
            }
            
            let view = viewController.view as? MAHighlightingView
